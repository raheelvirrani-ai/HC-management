...
def color_status(row):
    if row["Status"] == "Overstaffed":
        ...
        return ['background-color: #FFEB99']*len(row)  # light yellow
    else:
        return ['background-color: #99FF99']*len(row)  # light green

st.title("🍴 Dynamic Staffing & Labour Analysis")
st.write("Enter sales and staff numbers for each hour. Table and summary update automatically.")
